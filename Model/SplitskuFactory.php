...
class SplitskuFactory
{
    ...
    {
        return $this->_objectManager->create('Bsitc\Brightpearl\Model\Splitsku', $arguments, false);
    }
    ...
    public function getSplitSkuRows($sku) 
    {
		$data = [];
        $collection = $this->create()->getCollection()->addFieldToFilter('sku', $sku);
		 if ($collection->getSize()) {
			foreach ($collection as $row) {
				if($row->getSplitsku())
				{
					$recrods = explode(",",$row->getSplitsku());  
					foreach($recrods as $item){
						$record = trim($item);
						$data[$record] = $record;
					}
				}				
